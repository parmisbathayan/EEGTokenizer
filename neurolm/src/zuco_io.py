...
def _iter_hdf5(path):
    ...
    with h5py.File(path, "r") as handle:
        ...
        for content_ref, raw_ref in zip(
            np.asarray(data["content"]).flatten(), np.asarray(data["rawData"]).flatten()
        ):
...
def inspect_zuco(raw_dir, labels_csv, pattern="results*_SR.mat", min_samples=500):
    lookup = label_lookup(labels_csv)
    rows = []
    for path in find_subject_files(raw_dir, pattern):
        matched = raw_count = usable = total = too_short = unexpected = 0
        for content, raw in iter_subject_sentences(path):
            total += 1
            matched += normalize_text(content) in lookup
            if raw is not None:
                raw_count += 1
                usable += raw.shape[0] == 105 and raw.shape[1] >= min_samples
                too_short += raw.shape[1] < min_samples
                unexpected += raw.shape[0] != 105
        rows.append(
            {
                "subject": subject_from_path(path),
                "file": str(path),
