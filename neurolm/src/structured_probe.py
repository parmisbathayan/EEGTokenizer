"""Structured frozen-NeuroLM attention probe for bounded version 3."""
...
ALIGNED = "structured_neurolm_probe"
SHUFFLED = "structured_neurolm_probe_shuffled"
STRUCTURE_SHUFFLE = "structured_neurolm_structure_shuffle"
IMPLEMENTATION_VERSION = "structured-neurolm-v3.0"


@dataclass(frozen=True)
class StructuredProbeConfig:
    """One locked low-capacity probe and evaluation recipe for V3."""

    seeds: tuple = (42, 52, 62)
    n_splits: int = 5
    validation_fraction: float = 0.15
    expected_channels: int = 102
...
def _require_torch():
    try:
        ...
        raise ImportError("V3 training requires PyTorch; use the Colab notebook") from error
    return torch


def build_structured_probe(config=StructuredProbeConfig()):
    """Build a factorized channel-attention plus time-attention classifier."""

    torch = _require_torch()
    nn = torch.nn
    if config.hidden_size % 2:
        raise ValueError("hidden_size must be even for sinusoidal time positions")
    ...
    class AttentionPool(nn.Module):
        ...
        def forward(self, tokens, mask):
            ...
            return (tokens * attention.unsqueeze(-1)).sum(dim=1)

    class StructuredNeuroLMProbe(nn.Module):
        def __init__(self):
            super().__init__()
            self.input_norm = nn.LayerNorm(config.embedding_size)
            self.projection = nn.Sequential(
                nn.Linear(config.embedding_size, config.hidden_size),
                nn.GELU(),
                nn.Dropout(config.dropout),
            )
            self.channel_position = nn.Embedding(
                config.expected_channels, config.hidden_size
            )
            self.channel_pool = AttentionPool()
            self.time_pool = AttentionPool()
        ...
        @staticmethod
        def time_position(length, width, device, dtype):
        ...
        def forward(self, channel_tokens, time_tokens, time_mask):
            if channel_tokens.ndim != 3 or time_tokens.ndim != 3:
                raise ValueError("structured tokens must be batch x token x embedding")
            if channel_tokens.shape[1:] != (
                config.expected_channels,
                config.embedding_size,
            ):
                raise ValueError(
                    f"unexpected channel-token shape {tuple(channel_tokens.shape)}"
                )
            if time_tokens.shape[2] != config.embedding_size:
                raise ValueError(
                    f"expected time embedding {config.embedding_size}, got {time_tokens.shape[2]}"
                )
            channel_values = self.projection(self.input_norm(channel_tokens))
            channel_ids = torch.arange(
                config.expected_channels, device=channel_values.device
            )
            channel_values = channel_values + self.channel_position(channel_ids)[None]
            channel_mask = torch.ones(
                channel_values.shape[:2], dtype=torch.bool, device=channel_values.device
            )
            time_values = self.projection(self.input_norm(time_tokens))
            time_values = time_values + self.time_position(
                time_values.shape[1],
                config.hidden_size,
                time_values.device,
                time_values.dtype,
            )[None]
            channel_summary = self.channel_pool(channel_values, channel_mask)
            time_summary = self.time_pool(time_values, time_mask)
            fused = torch.cat(
                [
                    channel_summary,
                    time_summary,
                    torch.abs(channel_summary - time_summary),
                    channel_summary * time_summary,
                ],
                dim=1,
            )
            return self.classifier(fused)

    return StructuredNeuroLMProbe()
...
def make_bundle_examples(
    ...
    for target_id, target_label, source_id in zip(
        ...
        if not source_records:
            raise ValueError(f"no records for source sentence {source_id}")
...
def _stable_permutation(length, subject, sentence_id, seed, kind):
    payload = f"{subject}|{sentence_id}|{seed}|{kind}".encode()
...
class StructuredCollator:
    def __init__(self, config=StructuredProbeConfig(), structure_shuffle=False, seed=0):
        self.config = config
        self.structure_shuffle = structure_shuffle
        self.seed = seed

    def __call__(self, batch):
        torch = _require_torch()
        max_time = max(example.record.time_tokens.shape[0] for example in batch)
        channels = np.empty(
            (
                len(batch),
        ...
        times = np.zeros(
            (len(batch), max_time, self.config.embedding_size), dtype=np.float32
        )
        time_mask = np.zeros((len(batch), max_time), dtype=bool)
        for row, example in enumerate(batch):
            channel_values = np.asarray(example.record.channel_tokens, dtype=np.float32)
            time_values = np.asarray(example.record.time_tokens, dtype=np.float32)
            if channel_values.shape != (
                self.config.expected_channels,
                self.config.embedding_size,
            ):
                raise ValueError(f"invalid channel-token shape {channel_values.shape}")
            if time_values.ndim != 2 or time_values.shape[1] != self.config.embedding_size:
                raise ValueError(f"invalid time-token shape {time_values.shape}")
            if self.structure_shuffle:
                channel_values = channel_values[
                    _stable_permutation(
                        len(channel_values),
                        example.record.subject,
                        example.record.sentence_id,
                        self.seed,
                        "channel",
                    )
                ]
            ...
            channels[row] = channel_values
            times[row, : len(time_values)] = time_values
            time_mask[row, : len(time_values)] = True
        return {
            "channel_tokens": torch.from_numpy(channels),
            "time_tokens": torch.from_numpy(times),
...
def _loader(examples, config, shuffle, seed, structure_shuffle=False):
    torch = _require_torch()
    ...
    return torch.utils.data.DataLoader(
        ...
        shuffle=shuffle,
        num_workers=0,
        collate_fn=StructuredCollator(config, structure_shuffle, seed),
        generator=generator,
...
def _aggregate_predictions(model, loader, device):
    ...
    with torch.inference_mode():
        for batch in loader:
            logits = model(
                batch["channel_tokens"].to(device, non_blocking=True),
                batch["time_tokens"].to(device, non_blocking=True),
            ...
            for sentence_id, label_index, probability in zip(
                ...
                if sentence_id in labels and labels[sentence_id] != label:
                    raise ValueError(f"conflicting label for target sentence {sentence_id}")
...
def _train_one_model(
    ...
    _set_seed(seed)
    model = build_structured_probe(config).to(device)
    ...
    if best_state is None:
        raise RuntimeError("V3 training produced no finite validation result")
    model.load_state_dict(best_state)
    return model, history, best_epoch, best_score


def bootstrap_alignment_delta(predictions, config=StructuredProbeConfig(), seed=2026):
    from sklearn.metrics import f1_score
...
def gate_report(metrics, delta, config=StructuredProbeConfig()):
    ...
    decision = {
        "green": "GREEN — eligible for later tuning after V4 completes",
        "yellow": "YELLOW — suggestive only; record without tuning",
        "red": "RED — no alignment-specific V3 evidence; do not tune",
    }[status]
    return {
...
def smoke_test_structured_probe(records, config=StructuredProbeConfig(), device="cuda"):
    torch = _require_torch()
    sentence_ids, y = sentence_table(records)
    selected_ids = sentence_ids[: min(4, len(sentence_ids))]
    examples = make_bundle_examples(records, selected_ids, y[: len(selected_ids)])
    batch = StructuredCollator(config)(examples[: min(4, len(examples))])
    resolved = torch.device(device if torch.cuda.is_available() else "cpu")
    model = build_structured_probe(config).to(resolved).eval()
    with torch.inference_mode():
        logits = model(
            batch["channel_tokens"].to(resolved),
            batch["time_tokens"].to(resolved),
            batch["time_mask"].to(resolved),
        )
    return {
        "device": str(resolved),
        "channel_input": list(batch["channel_tokens"].shape),
        "time_input": list(batch["time_tokens"].shape),
...
def evaluate_structured_probe(
    ...
    """Run/resume the complete V3 aligned, shuffled, and structure-control screen."""
    ...
    if resolved_device.type != "cuda":
        raise RuntimeError("V3 evaluation requires a Colab GPU runtime")
    ...
    if signature_path.exists():
        ...
        if existing.get("signature") != signature:
            raise RuntimeError(
                "V3 result directory belongs to a different data/configuration signature"
            )
    else:
    ...
    for seed in config.seeds:
        ...
        for fold, (train_position, test_position) in enumerate(
            ...
            for setup, shuffled in ((ALIGNED, False), (SHUFFLED, True)):
                produced = [setup, STRUCTURE_SHUFFLE] if not shuffled else [setup]
                marker = completion_dir / f"{setup}_seed{seed}_fold{fold}.json"
                if _fold_complete(
                    metrics, predictions, produced, seed, fold, len(test_ids), marker
                ):
                    print(f"Reused {setup} seed={seed} fold={fold}", flush=True)
                    continue
                metrics = _drop_fold_rows(metrics, produced, seed, fold)
                predictions = _drop_fold_rows(predictions, produced, seed, fold)
                ...
                test_loader = _loader(
                    test_examples, config, False, fit_seed + 60_000
                )
                ...
                if not shuffled:
                    ...
                    structure_ids, structure_truth, structure_probabilities = (
                        _aggregate_predictions(model, structure_loader, resolved_device)
                    )
                    structure_rows, structure_predictions = _prediction_rows(
                        STRUCTURE_SHUFFLE,
                        seed,
                        fold,
                        structure_ids,
                    ...
                    new_metrics.append(
                        {
                            "setup": STRUCTURE_SHUFFLE,
                            "seed": seed,
                            "fold": fold,
                            "best_epoch": best_epoch,
                            "validation_macro_f1": best_score,
                            **_metric_values(structure_truth, structure_predictions),
                        }
                    )
                    new_predictions.extend(structure_rows)
                metrics = pd.concat(
